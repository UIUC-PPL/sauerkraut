...
class CMakeBuild(build_ext):
    def build_flatbuffers(self):
        install_dir = os.path.abspath(
            os.path.join(self.build_temp, "flatbuffers-install")
        )
        try:
            subprocess.check_output(
                [os.path.join(install_dir, "bin", "flatc"), "--version"]
            )
            return None
        except OSError:
            print("Building Flatbuffers from source...")
            code_url = (
                "https://github.com/google/flatbuffers/archive/refs/tags/v24.3.25.zip"
            )
            code_dir = os.path.abspath(
                os.path.join(self.build_temp, "flatbuffers-source")
            )
            build_dir = os.path.abspath(
                os.path.join(self.build_temp, "flatbuffers-build")
            )
            ...
            if not os.path.exists(zip_path):
                raise RuntimeError(
                    f"Failed to download Flatbuffers source to {zip_path}"
                )
    ...
    def build_extension(self, ext):
        ...
        install_dir = os.path.dirname(os.path.abspath(self.get_ext_fullpath(ext.name)))
        ...
        cmake_args = [
            ...
            # Ensure CMake uses the same interpreter invoking setup.py.
            # `find_package(Python ...)` honors `Python_EXECUTABLE`.
            f"-DPython_EXECUTABLE={sys.executable}",
        ]

        if self._flatbuffers_dir:
            flatc = os.path.join(self._flatbuffers_dir, "bin", "flatc")
            cmake_args.extend(
                [
                    f"-DFLATBUFFERS_ROOT={self._flatbuffers_dir}",
                    f"-DFLATC_EXECUTABLE={flatc}",
                ]
            )
    ...
    def _get_python_library(self):
        ...
        library_names = [
            ...
            f"libpython{py_ver}d.so",
            f"libpython{py_ver}m.so",
            f"python{py_ver}.dll",
        ]
